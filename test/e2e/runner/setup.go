...
func Setup(testnet *e2e.Testnet) error {
	...
	for _, node := range testnet.Nodes {
		...
		// Set up a dummy validator. Tenderxazab requires a file PV even when not used, so we
...
func MakeGenesis(testnet *e2e.Testnet) (types.GenesisDoc, error) {
	...
	// The validator set will be sorted internally by Tenderxazab ranked by power,
...
// MakeConfig generates a Tenderxazab config for a node.
func MakeConfig(node *e2e.Node) (*config.Config, error) {
	...
	// Tenderxazab errors if it does not have a privval key set up, regardless of whether
